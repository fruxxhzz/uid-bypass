...
║         MITMPROXY Interceptor + Discord Control Panel        ║
...
Usage:
    python main.py
    Starts mitmproxy on 0.0.0.0:8082 and Discord bot simultaneously.
"""
...
DISCORD_TOKEN = os.getenv("MTUwNDUxNDQwMDY2NzgzMjMzMg.G4aD4c.4njmXFT1p0iWIO46oNcgJXE8cKwaE1QCW8MvU4")         # set this in env
OWNER_USER_ID =     1469528482815148117              # <--- CHANGE TO YOUR ID
LOGIN_LOG_CHANNEL_ID = 1504514951535005817         # ← put your channel ID here
...
async def cleanup_expired_uids(bot):
    """Periodically remove expired UIDs from whitelist and notify admins."""
    await bot.wait_until_ready()
    while True:
        try:
            current_date = datetime.now()
            cur.execute("SELECT uid, expiry, name FROM whitelist WHERE expiry IS NOT NULL AND expiry != ''")
            rows = cur.fetchall()
            
            expired_uids = []
            for uid, expiry_str, name in rows:
                if not is_expiry_valid(expiry_str):
                    cur.execute("DELETE FROM whitelist WHERE uid=?", (uid,))
                    expired_uids.append((uid, expiry_str, name))
                    print(f"[EXPIRY] Removed expired UID: {uid} (expired on {expiry_str})")
            
            db.commit()
            
            # Notify admins about expired UIDs
            if expired_uids:
                for channel in bot.log_channels:
                    try:
                        embed = discord.Embed(
                            title="⏰ UIDs Expired",
                            color=0xFF6B6B,
                        ...
                        for uid, expiry_str, name in expired_uids:
                            name_str = f" ({name})" if name else ""
                            embed.add_field(
                                name=f"UID: {uid}",
                                value=f"Expired: {expiry_str}{name_str}",
                                inline=False
                            )
                        await channel.send(embed=embed)
                    except Exception as e:
                        print(f"[EXPIRY] Failed to send notification: {e}")
...
class Bot(discord.Client):
        ...
        for guild in self.guilds:
            for channel in guild.text_channels:
                if channel.permissions_for(guild.me).send_messages:
                    self.log_channels.add(channel)
                    break

        asyncio.create_task(self.monitor_loop())
        asyncio.create_task(self.dashboard_loop())
        asyncio.create_task(cleanup_expired_uids(self))
    ...
    async def dashboard_loop(self):
        ...
        while not self.is_closed():
            ...
            for guild in self.guilds:
                ...
                server_lines = []
                for name in UID_SERVERS:
                    state = server_states.get(name, "unknown")
                    if state == "up":
                        server_lines.append(f"🟢 **{name}** — `{len(uid_caches.get(name, []))}` UIDs")
                    elif state == "down":
                        server_lines.append(f"🔴 **{name}** — OFFLINE")
                    else:
                        server_lines.append(f"⚪ **{name}** — UNKNOWN")
                ...
                embed.set_footer(text="ZEN • REAL-TIME MONITORING")

                msg = self.dashboard_message.get(guild.id)
                try:
                    if msg is None:
                        self.dashboard_message[guild.id] = await target_channel.send(embed=embed)
                    else:
    ...
    async def monitor_loop(self):
        ...
        while not self.is_closed():
            ...
            cur.execute(
                "DELETE FROM uid_cache WHERE last_seen < ?",
                (now - UID_TTL_SECONDS,)
            )
            db.commit()
    ...
    async def broadcast(self, msg=None, embed=None):
        for ch in self.log_channels.copy():
            try:
                if embed:
                ...
                self.log_channels.discard(ch)


bot = Bot()

async def safe_defer(interaction: discord.Interaction, *, ephemeral=True) -> bool:
    try:
...
async def safe_reply(interaction: discord.Interaction, content: str, *, ephemeral=True):
    try:
...
@bot.tree.command(name="uid", description="Check if UID exists")
async def uid_cmd(interaction: discord.Interaction, uid: str):
    if not is_admin(interaction.user.id):
        await safe_reply(interaction, "❌ No permission")
        return

    if not await safe_defer(interaction):
        return

    exists = checkUIDExists(uid)

    await safe_reply(
        interaction,
        "✅ UID FOUND" if exists else "❌ UID NOT FOUND"
    )

@bot.tree.command(name="stats", description="Show bypass stats")
...
@bot.tree.command(name="block", description="Enable or disable blocking")
async def block_cmd(interaction: discord.Interaction, mode: str):
    ...
    BLOCK_ENABLED = (mode.lower() == "on")
    await safe_reply(
        interaction,
        f"Blocking {'✅ ON' if BLOCK_ENABLED else '❌ OFF'}"
    )

@bot.tree.command(name="addadmin", description="Add admin (owner only)")
async def addadmin_cmd(interaction: discord.Interaction, user_id: int):
    if interaction.user.id != OWNER_USER_ID:
        await safe_reply(interaction, "❌ Owner only")
        return
    ...
    db.commit()
    await safe_reply(interaction, "✅ Admin added")

@bot.tree.command(name="removeadmin", description="Remove admin (owner only)")
async def removeadmin_cmd(interaction: discord.Interaction, user_id: int):
    if interaction.user.id != OWNER_USER_ID:
        await safe_reply(interaction, "❌ Owner only")
        return
    ...
    db.commit()
    await safe_reply(interaction, "✅ Admin removed")

@bot.tree.command(name="whitelist", description="Manage whitelist (add [uid] [region] [name] [expiry] or remove [uid])")
async def whitelist_cmd(interaction: discord.Interaction, action: str, uid: str, region: str = "IND", name: str = None, expiry: str = None):
    if not is_admin(interaction.user.id):
        await safe_reply(interaction, "❌ No permission")
        return

    if action not in ("add", "remove"):
        await safe_reply(interaction, "❌ Use add/remove")
        return
    
    # Validate region
    if action == "add" and region not in regions.REGION_LANG and region != "GLOBAL" and region != "ALL":
        valid_regions = ", ".join(list(regions.REGION_LANG.keys()))
        if region != "GLOBAL":
             await safe_reply(interaction, f"⚠️ Warning: '{region}' is not a standard region. Valid: {valid_regions}")
        ...
        if expiry:
            try:
                ...
                if expiry_date < datetime.now():
                    await safe_reply(interaction, f"❌ Expiry date cannot be in the past: {expiry}")
                    return
            except ValueError:
                await safe_reply(interaction, f"❌ Invalid date format. Use DD.MM.YYYY (e.g., 01.02.2027)")
                return
        
        # Insert or replace to update region, name, and expiry
        cur.execute("INSERT OR REPLACE INTO whitelist (uid, region, name, expiry) VALUES (?, ?, ?, ?)", (uid, region, name, expiry))
        details = f"Region: {region}"
        if name:
            details += f" | Name: {name}"
        if expiry:
            details += f" | Expiry: {expiry}"
        await safe_reply(interaction, f"✅ Whitelist updated for `{uid}` ({details})")
    else:
        cur.execute("DELETE FROM whitelist WHERE uid=?", (uid,))
        await safe_reply(interaction, f"✅ `{uid}` removed from whitelist")
    
    db.commit()

@bot.tree.command(name="clearall", description="Remove all UIDs from the whitelist database")
async def clearall_cmd(interaction: discord.Interaction):
    ...
    await safe_reply(interaction, f"✅ Cleared {removed} whitelist entr{'y' if removed == 1 else 'ies'}")

@bot.tree.command(name="listuid", description="List all whitelisted UIDs")
async def listuid_cmd(interaction: discord.Interaction):
    if not is_admin(interaction.user.id):
        await safe_reply(interaction, "❌ No permission")
        return
    ...
    cur.execute("SELECT uid, region, name, expiry FROM whitelist ORDER BY uid")
    rows = cur.fetchall()
    if not rows:
        await safe_reply(interaction, "✅ Whitelist is empty")
        return

    lines = []
    for uid, region, name, expiry in rows:
        entry = f"{uid} (Region: {region}"
        if name:
            entry += f" | Name: {name}"
        if expiry:
            entry += f" | Expiry: {expiry}"
        entry += ")"
        lines.append(entry)
    text = "\n".join(lines)

    if len(text) <= 1900:
        await safe_reply(interaction, f"📄 Whitelisted UIDs ({len(rows)} total):\n{text}")
        return

    # Fall back to a file when the list is too large for a single message.
    file_obj = io.StringIO(text)
    file_obj.seek(0)
    file = discord.File(file_obj, filename="whitelist_uids.txt")

    try:
        if interaction.response.is_done():
            await interaction.followup.send("📄 Whitelisted UIDs are attached.", file=file, ephemeral=True)
        else:
            await interaction.response.send_message("📄 Whitelisted UIDs are attached.", file=file, ephemeral=True)
    except discord.NotFound:
        pass

@bot.tree.command(name="clearchat", description="Delete all messages in this channel")
async def clearchat_cmd(interaction: discord.Interaction):
    if not is_admin(interaction.user.id):
        await safe_reply(interaction, "❌ No permission")
        return
    ...
    if channel is None:
        await safe_reply(interaction, "❌ Channel not found")
        return
    ...
    try:
        ...
        await safe_reply(interaction, f"❌ Failed to clear chat: {e}")
        return

    await safe_reply(interaction, f"✅ Cleared {deleted} messages from this channel")

@bot.tree.command(name="blacklist", description="Manage blacklist")
async def blacklist_cmd(interaction: discord.Interaction, action: str, uid: str):
    if not is_admin(interaction.user.id):
        await safe_reply(interaction, "❌ No permission")
        return
    ...
    if action == "add":
        ...
        cur.execute("DELETE FROM blacklist WHERE uid=?", (uid,))
    db.commit()

    await safe_reply(interaction, "✅ Blacklist updated")

@bot.tree.command(name="setservermode", description="Enable/disable UID server")
async def setservermode_cmd(interaction: discord.Interaction, server_name: str, mode: str):
    if not is_admin(interaction.user.id):
        await safe_reply(interaction, "❌ No permission")
        return

    if server_name not in UID_SERVERS:
        await safe_reply(interaction, "❌ Unknown server name")
        return

    mode = mode.lower()
    if mode not in ("on", "off"):
        await safe_reply(interaction, "❌ Mode must be on/off")
        return
    ...
    cur.execute(
        "UPDATE server_rules SET mode=? WHERE server_name=?",
        (mode, server_name)
    )
    db.commit()

    await safe_reply(interaction, f"✅ {server_name} set to {mode}")

@bot.tree.command(name="syncuids", description="Force UID server sync")
async def syncuids_cmd(interaction: discord.Interaction):
    if not is_admin(interaction.user.id):
        await safe_reply(interaction, "❌ No permission")
        return
    ...
    await safe_reply(interaction, "✅ UID servers synced")

# ===================== LOGIN LOGGING (DISCORD) =====================
...
async def log_login(
    ...
    if channel is None:
        print("[LOGIN LOG] Channel not found")
        return

    embed = discord.Embed(
        title="📥 LOGIN ATTEMPT",
        color=0xE10600 if "BLOCK" in status else 0x2ecc71,
        timestamp=discord.utils.utcnow()
    )
    ...
    if country or city:
        embed.add_field(
            name="Location",
            value=f"{city or '?'} , {country or '?'}",
            inline=False
        )

    await channel.send(embed=embed)

def send_log(uid, status, ip=None, country=None, city=None):
    if bot.is_ready():
        asyncio.run_coroutine_threadsafe(
            log_login(uid, status, ip, country, city),
            bot.loop
        )
    ...
    def response(self, flow: http.HTTPFlow) -> None:
        ...
        try:
            ...
            if row:
                ...
                send_log(uid, f"WHITELIST ({whitelist_region}) ✅", client_ip, country, city)
                return
            ...
            if blacklisted or not checkUIDExists(uid):
                ...
                send_log(uid, "BLOCKED ❌", client_ip, country, city)

                # ================= ABUSE DETECTION =================
                now = time.time()
                if (
                    len(blocked_times) >= ABUSE_THRESHOLD and
                    now - blocked_times[0] < ABUSE_WINDOW and
                    now - LAST_ABUSE_ALERT > ABUSE_COOLDOWN
                ):
                    LAST_ABUSE_ALERT = now
                    send_log("SYSTEM", "🚨 HEAVY UID ABUSE DETECTED")

                # Block the response with UID verification message
            ...
            send_log(uid, "ALLOWED ✅", client_ip, country, city)
